...
def create_branch(branch: schemas.BranchCreate, db: Session = Depends(get_db)):
    ...
    if db_branch is None:
        raise HTTPException(status_code=400, detail="Branch_id is already exists")
    return db_branch
...
def create_employee(employee: schemas.EmployeeCreate, db: Session = Depends(get_db)):
    ...
    if db_employee is None:
        raise HTTPException(status_code=400, detail="Employee_id is already exists")
    return db_employee
...
def update_employee (employee_id: int ,employee : schemas.EmployeeCreate, db : Session = Depends(get_db)):
    db_update = crud.update_employee(db , employee_id=employee_id, employee = employee)
    if db_update is None:
        raise HTTPException(status_code=404, detail="Employee not found")
    return db_update
...
def create_tool(tool: schemas.ToolCreate, db: Session = Depends(get_db)):
    ...
    if db_tool is None:
        raise HTTPException(status_code=400, detail="Tool_id is already exists")
    return db_tool
...
def create_item(item: schemas.ItemCreate, db: Session = Depends(get_db)):
    ...
    if db_item is None:
        raise HTTPException(status_code=400, detail="Item_id is already exists")
    return db_item
...
# Add Api functions
@app.post(f"/{adds}""/", response_model=schemas.Add)
def create_add( add:schemas.AddCreate,db: Session = Depends(get_db)):
    db_add = crud.create_add(db, add = add)
    if db_add is None:
     raise HTTPException(status_code=400, detail="Add_id is already exists")
    return db_add

@app.get(f"/{adds}""/{add_id}", response_model=schemas.Add)
...
def create_book(book: schemas.BookCreate, db: Session = Depends(get_db)):
    ...
    if db_book is None:
        raise HTTPException(status_code=400, detail="Book_id is already exists")
    return db_book
...
def create_check_in_out(check_in_out: schemas.Check_in_out, db: Session = Depends(get_db)):
    ...
    if db_check_in_out is None:
        raise HTTPException(status_code=400, detail="Check_in_out_id is already exists")
...
def update_check_in_out (check_id:int ,check_in_out : schemas.Check_in_out, db : Session = Depends(get_db)):
    db_update = crud.update_check_in_out(db , check_id=check_id,check_in_out= check_in_out)
    if db_update is None:
        raise HTTPException(status_code=404, detail="Check_in_out not found")
    return db_update
...
def create_comment(comment: schemas.CommentCreate, db: Session = Depends(get_db)):
    ...
    if db_comment is None:
        raise HTTPException(status_code=400, detail="Comment_id is already exists")
    return db_comment
